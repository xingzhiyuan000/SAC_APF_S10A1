...
ACTION_DIM = 1              # 输出动作维度: 1排斥力权重大小-q4
...
df.to_excel(data+f"Reward-sac-apf-S10A1-{timestamp}.xlsx", index=False, header=False)  # 导出 Excel

# 【阶段成功】导出为EXCEL
df_success = pd.DataFrame(SUCCESS_BUFFER)
df_success.to_excel(data+f"success-sac-apf-S10A1-{timestamp}.xlsx", index=False, header=False)
...
if PLOT_REWARD:
    ...
    plt.savefig(image + f"Reward-sac-apf-S10A1-{timestamp}.png", format='png')
    ...
    plt.savefig(image + f"loss-sac-apf-S10A1-{timestamp}.png", format='png')
    ...
    plt.tight_layout()  # 自动调整子图间距
    plt.savefig(image + f"step-Q-sac-apf-S10A1-{timestamp}.png", format='png')
    plt.show()
